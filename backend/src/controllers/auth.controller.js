...
export const signup = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    // If email fails, still send response but log the error
...
export const logout = (req, res) => {
  res.cookie('jwt', 'loggedout', {
    ...
    sameSite: 'strict',
    path: '/',
    domain: process.env.NODE_ENV === 'production' ? '.melbournewineschool.com' : undefined
  });
  
  res.status(200).json({ status: 'success' });
};

// Helper function to set JWT as HTTP-only cookie
